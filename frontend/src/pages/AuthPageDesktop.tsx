...
function BrandPanel() {
  return (
    <div className="relative hidden lg:flex w-[52%] max-w-[760px] shrink-0 bg-gradient-to-br from-[#fafbfd] to-[#f0f4f8] flex-col justify-center p-12 xl:p-16 overflow-hidden">
      {/* 柔和渐变光斑（品牌色，缓慢漂浮） */}
      ...
      {/* 中间左侧动漫人物装饰 */}
      <div className="absolute top-1/4 left-4 pointer-events-none animate-float-medium opacity-85">
        <img 
          src="/anime/bg1_transparent.png" 
          alt="Anime character decoration" 
          className="w-36 h-48 object-contain drop-shadow-2xl"
        />
      </div>

      {/* 底部右侧动漫人物装饰 */}
      <div className="absolute bottom-8 right-12 pointer-events-none animate-float-fast opacity-90">
        <img 
          src="/anime/bg_transparent.png" 
          alt="Anime character decoration" 
          className="w-44 h-64 object-contain drop-shadow-2xl"
        />
      </div>

      <div className="relative max-w-lg z-10">
        <div className="space-y-6">
          <h2 className="text-[36px] leading-[1.2] font-bold text-gray-900 xl:text-[42px]">
            一个为跨性别群体打造的
            <br />
            温暖通信与社区平台
          </h2>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            {["即时通信", "情绪日记", "资源中心", "音乐治愈"].map((chip) => (
              <span
                key={chip}
                className="inline-flex items-center px-4 py-2 rounded-full border border-gray-200/60 bg-white/80 text-sm text-gray-700 shadow-sm hover:shadow-md transition-shadow"
              >
                {chip}
              </span>
            ))}
...
function AuthModeSwitch({
  ...
  return (
    <div className="relative flex rounded-xl bg-gray-100/50 p-1">
      <div
        className="absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] rounded-lg bg-white shadow-sm transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1)]"
        style={{
      ...
      {[
        ...
        return (
          <button
            ...
            className={cn(
              "relative z-10 flex-1 h-9 rounded-lg text-sm font-medium transition-colors duration-300",
...
function LoginMethodPills({
  ...
  return (
    <div className="relative flex rounded-lg bg-gray-100/50 p-1">
      <div
        className="absolute top-1 bottom-1 left-1 w-[calc((100%-8px)/3)] rounded-md bg-white shadow-sm transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1)]"
        style={{
      ...
      {methodOptions.map((option) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative z-10 flex-1 h-8 rounded-md flex items-center justify-center gap-1.5 text-xs transition-colors duration-300",
              active
